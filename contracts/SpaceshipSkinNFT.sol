...
interface ISOLA {
    ...
    function adminTransfer(address recipient, uint256 amount) external;
    ...
    Pausable
{
    ...
    mapping(address => bool) public blockMarketList;
    ...
    /**
     * @dev
     * `maxPerTransaction ` refers to how much a minter can mint at a time.
     */
    ...
    modifier isBlockMarket(address _market_address) {
        require(
            blockMarketList[_market_address] == false,
            "Pixel Runs: You can not list your nft on this Marketplace."
        );
        _;
    }
        ...
        returns (TokenOwnership memory)
    {
        ...
        if (tokenId >= maxPerTransaction) {
            lowestTokenToCheck = tokenId - maxPerTransaction + 1;
        }
...
contract SpaceshipSkinNFT is Ownable, ERC721A {
    ...
    /**
     * @dev TheSolaVerse: SpaceShip NFT
     */
    constructor(address _sola_address, address _processor_wallet)
        ERC721A("Spaceship Skins NFT", "SSN")
    {
        solaTokenAddress = _sola_address;
    ...
    /**
     * @dev TheSolaVerse: Modifier for signature verification
     */
    modifier isValidHash(
        ...
        require(
            ECDSA.recover(hash, _signature) == processor_wallet,
            "verify failed."
        );
        _;
    }
    ...
    /**
     * @dev TheSolaVerse: Update the BaseURI for the reveals.
     */
    ...
    /**
     * @dev TheSolaVerse: Get the Base URI.
     */
        ...
        )
    {
        ...
        emit ClaimNFT(msg.sender, num_tokens);
    }

    /**
     * @dev TheSolaVerse: Mint nft from the marketplace.
     */
    function buyNFT(
        string[] calldata _skins,
        string[] calldata _uuids,
        ...
        )
    {
        ...
        emit BuyNFT(msg.sender, num_tokens);
    }

    /**
     * @dev TheSolaVerse: Set the processor wallet for the signature verification
     */
    function setProcessorWallet(address _processor_wallet) external onlyOwner {
        processor_wallet = _processor_wallet;
    }
     ...
     */
    function setTokenAddress(address _token_address) external onlyOwner {
        solaTokenAddress = _token_address;
    }
    ...
    function concatArrays(
        ...
        for (uint256 i = 0; i < _firstArray.length; i++) {
            output = abi.encodePacked(output, _firstArray[i], _secondArray[i]);
        }
    ...
    /**
     * @dev TheSolaVerse: Save toeknID with UUID
     */
    function saveTokenID(uint256 _max_token_id, string[] memory _uuids)
        internal
    {
        uint256 loop = _uuids.length - 1;
        for (uint256 i = 0; i <= loop; i++) {
            token_uuids[_uuids[loop - i]] = _max_token_id - i;
        }
    }

    /**
     * @dev TheSolaVerse: Get the tokenID from UUID
     */
    function getTokenIdFromUUID(string memory _uuid)
        external
        view
        returns (uint256)
    {
